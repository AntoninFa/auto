...
export class AuthController {
    ...
    /**
     * Einloggen mit `@UseGuards`.
     *
     * Statuscode `200` (`OK`) bei Erfolgreichem Login.
     *
     * Statuscode `401` (`Unauthorized`) bei Fehlgeschlagenem login.
     *
    ...
    @ApiOkResponse({ description: '(200) Erfolgreich eingeloggt.' })
    @ApiUnauthorizedResponse({
        description: '(401) Login Fehlgeschlagen.',
    })
    async login(
        @Req() req: Request,
        @Res() res: Response,
